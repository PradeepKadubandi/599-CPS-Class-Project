...
% <docid:driving_ug#bvgu_ya-2 Coordinate Systems in Automated Driving
% System Toolbox>.
...
% # *Path Smoothing:* Smooth the reference path by fitting splines using
% |<matlab:openExample('driving/ParkingValetExample','supportingFile','HelperCubicSplineFit.m')
% HelperCubicSplineFit>|.
% # *Trajectory Generation:* Convert the smoothed path into a trajectory by
% generating a speed profile using |<matlab:openExample('driving/ParkingValetExample','supportingFile','HelperSpeedProfileGenerator.m') HelperSpeedProfileGenerator>|.
...
% computes the steering angle to control the heading of the vehicle and 
...
% segment using |<matlab:openExample('driving/ParkingValetExample','supportingFile','helperGoalChecker.m') helperGoalChecker>|.
% 
...
% Create a
% |<matlab:openExample('driving/ParkingValetExample','supportingFile','HelperCubicSplineFit.m')
% HelperCubicSplineFit>| object to fit a parametric cubic spline that
% passes through all the transition points in the reference path. The
% spline approximately matches the starting and ending directions with the
% starting and ending heading angle of the vehicle. The
% |<matlab:openExample('driving/ParkingValetExample','supportingFile','HelperCubicSplineFit.m')
% HelperCubicSplineFit>| object uses the |<docid:matlab_ref#bvjdpi3
% spline>| function for curve fitting.
...
% |<matlab:openExample('driving/ParkingValetExample','supportingFile','HelperSpeedProfileGenerator.m') HelperSpeedProfileGenerator>| 
% class generates such a speed profile.
%
...
%%
% Create a
% |<matlab:openExample('driving/ParkingValetExample','supportingFile','HelperSpeedProfileGenerator.m')
% HelperSpeedProfileGenerator>| object to generate the speed profile based
...
plotSpeedProfile(refPathLengths, refSpeeds, maxSpeed)
...
% back-in.
parkMotionPlanner.MinTurningRadius   = 12; % meters
...
[poses, directions] = interpolate(refPath, lengths);

% Assign poses and directions to the spline fitting object
splineFitter.Poses      = poses;
splineFitter.Directions = directions;
...
% # Realize different parking behavior by using different motion planner
% settings.
